...
router.post('/', verifyToken, upload.single('image'), (req, res) => {
  ...
  db.query('SELECT * FROM items WHERE id = ?', [item_id], (err, results) => {
    ...
    // ✅ ENUM values: 'Pending', 'Approved', 'Rejected' — capital first letter
    ...
    db.query(sql, [item_id, full_name, email, phone || '', image, message || ''], (err2) => {
      ...
      db.query(
        ...
        [req.user.id, `New claim by ${full_name} on "${item.item_name}"`],
        (err3) => { if (err3) console.error('Admin notif error:', err3.sqlMessage); }
      );

      // ✅ Item owner notification
      if (item.user_id) {
        db.query(
          'INSERT INTO notifications (user_id, message, is_read, for_admin) VALUES (?, ?, 0, 0)',
          [item.user_id, `Someone claimed your item "${item.item_name}". Admin is reviewing.`],
          (err4) => { if (err4) console.error('Owner notif error:', err4.sqlMessage); }
        );
      }
